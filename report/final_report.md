...
Two methods are used. A finite Markov chain gives the stationary long-run performance of each fixed policy, evaluated numerically to a tolerance of `1e-14`. Repeated Monte Carlo simulation then checks that calculation. The baseline experiment compares 45 feasible policies, using 200 independent replications that discard 365 warm-up days before measuring 365 days. Under the baseline assumptions, `(1, 12)` has the lowest average daily cost: `19.66`. Its simulated mean is also `19.66`, with a 95% interval of `[19.61, 19.71]`; the stationary result lies inside that interval. The policy has a `10.69%` stockout rate and a `93.91%` fill rate.

The cost minimum is not automatically the best management choice. When an illustrative `97%` fill-rate constraint is imposed, `(2, 12)` becomes the lowest-cost feasible policy. It costs `20.11` per day, only `0.46` more than the unconstrained minimum, while reducing the stockout rate to `5.93%`. Four demand distributions add a second stress test. The cost optimum remains `(1, 12)`, but the policy required to meet the 97% scenario target moves from `(2, 12)` under steady or baseline demand to `(3, 12)` under volatile demand and `(4, 12)` during a promotion peak. The value of the work lies in this explicit decision process: assumptions, calculations, and operational trade-offs remain open to inspection.
...
| `policy_evaluation_summary.csv` | Stationary and simulated metrics for all 45 policies |
...
| `demand_scenario_policy_evaluation.csv` | Stationary metrics for all 180 scenario-policy combinations |
...
Within the tested baseline grid, `(s, S) = (1, 12)` has the lowest stationary expected cost: `19.6568` per day. After warm-up, simulation gives `19.6571`, with a 95% interval of `[19.6053, 19.7089]`. The difference between the two estimates is only `0.0003` cost units per day, and the stationary value lies inside the simulated interval. The analytical and computational methods now provide a clean validation check.
...
The heatmap compares stationary average daily cost across all 45 policies. Darker cells indicate lower cost, and the gold outline marks `(1, 12)`. If `S` is too low, shortages and repeated replenishment become expensive. If `s` or `S` is too high, service improves but holding cost accumulates. The minimum lies between those two extremes.
...
Cost sensitivity does not answer the whole question because two demand processes can have similar means and very different tail risk. The model therefore repeats the 45-policy search under four discrete distributions. Baseline mixed demand preserves the original assumptions. Steady demand concentrates probability near three units. Volatile demand keeps mean demand at `3.00` but shifts more probability into the tails. Promotion-peak demand raises the mean to `4.79` and extends possible daily demand to eight units. Costs, lead time, and the policy grid remain unchanged, so the comparison isolates the effect of changing demand.

| Demand scenario | `E[D]` | `Var(D)` | Cost optimum | Daily cost |
| --- | ---: | ---: | --- | ---: |
| Baseline mixed | 3.13 | 2.43 | `(1, 12)` | 19.66 |
| Steady | 3.00 | 0.90 | `(1, 12)` | 18.76 |
| Volatile | 3.00 | 4.60 | `(1, 12)` | 19.52 |
| Promotion peak | 4.79 | 3.07 | `(1, 12)` | 27.23 |

The cost-only optimum remains `(1, 12)` in all four scenarios. That is a result, not an assumption: each row comes from a fresh enumeration of all 45 policies. The stable choice indicates that the baseline cost structure, especially the fixed ordering charge, continues to favour waiting and replenishing to the upper grid limit. Performance is not stable, however. At the same mean demand of `3.00`, the fill rate falls from `96.01%` under steady demand to `92.18%` under volatile demand. Variance changes the service risk even when average demand does not.

| Demand scenario | Cost-optimum fill rate | Lowest-cost 97% policy | Constrained fill rate |
| --- | ---: | --- | ---: |
| Baseline mixed | 93.91% | `(2, 12)` | 97.09% |
| Steady | 96.01% | `(2, 12)` | 98.84% |
| Volatile | 92.18% | `(3, 12)` | 98.13% |
| Promotion peak | 89.57% | `(4, 12)` | 98.15% |

The illustrative service target exposes a clearer policy response. Greater volatility requires an earlier trigger, and promotion-peak demand moves the trigger to four units. This does not prove that 97% is the right target for a real retailer. It shows how a declared service requirement changes the feasible set and how the demand distribution changes the least-cost policy within that set. For each scenario, 200 warm-up-adjusted Monte Carlo replications independently check the selected cost optimum; every stationary cost lies inside its simulated 95% interval.
...
Cost and service also need separate attention. The lowest-cost baseline policy has a stockout rate above ten percent, which may be difficult to defend when availability matters. Policy `(2, 12)` is the minimum-cost solution after the illustrative `97%` fill-rate constraint is imposed. The extra `0.46` per day, or `2.32%`, buys a `4.75` percentage-point reduction in stockout probability. Under more volatile or promotion-peak demand, the same target requires an even earlier trigger. The numerical minimum becomes an input to management judgement rather than a substitute for it.
...
This study applies an `(s, S)` replenishment rule to a category-neutral ecommerce case with stochastic demand. A finite Markov chain provides stationary long-run metrics evaluated to a declared numerical tolerance, and warm-up-adjusted Monte Carlo simulation independently checks the implementation. Within the tested baseline grid, `(1, 12)` is the unconstrained cost minimum at `19.66` per day. Once an illustrative `97%` fill-rate requirement is imposed, `(2, 12)` becomes the lowest-cost feasible choice at `20.11` per day. Repeating the search under four demand distributions shows that the cost optimum is stable in this grid, while the service-constrained reorder point rises to three under volatile demand and four during the promotion peak.
